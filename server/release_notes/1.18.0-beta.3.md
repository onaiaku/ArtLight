# ArtLight Server 1.18.0-beta.3 - 2026-06-28

Notice: ArtLight Server may trigger false-positive antivirus alerts.

This release fixes application saving, which could silently fail in recent 1.18.0 prereleases, and makes the ArtLight Server virtual display the default capture path for fresh installs.

## Changes
- Fresh installs now default to the ArtLight Server Display Driver with per-client exclusive virtual displays, while SudoVDA remains available as an explicit rollback option. Existing config keys are preserved, with the visible control inverted to "Use SudoVDA", and all locales describe the new default/rollback behavior.
