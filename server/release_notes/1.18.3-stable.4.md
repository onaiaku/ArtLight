# ArtLight Server 1.18.3-stable.4 - 2026-08-05
...
> [!IMPORTANT]
> Artemis clients must enable **Use virtual display** to use a virtual display; Artemis leaves this option off by default. ArtLight Server now reports virtual-display capability and driver readiness so it can distinguish an unsupported host from a recoverable unavailable driver.

## Changes
- Added virtual-display driver-readiness status to client discovery so compatible clients can distinguish a recoverable virtual-display launch from an unsupported host.
