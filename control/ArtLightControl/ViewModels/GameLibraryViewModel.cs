        ...
        public async Task ToggleHostAssetsAsync()
        {
            ...
            try
            {
                if (_hostAssetsApplied)
                {
                    ...
                    {
                        ShowStatus("Could not restore host tiles. Is the ArtLightControl service running?", isError: true);
                    }
                }
                else
                {
                    ...
                    {
                        ShowStatus("Could not replace host tiles. Is the ArtLightControl service running?", isError: true);
                    }
                }
            }
            catch (Exception ex)
            ...
            try
            {
                ...
                if (!string.IsNullOrEmpty(entry.ExePath))
                {
                    Process.Start(new ProcessStartInfo
                    {
                        ...
                        UseShellExecute = true,
                        // ⚠️ Without this the game inherits *ArtLightControl's* working directory,
                        ...
                        // "Could not find: 'C:/Program Files/ArtLightControl/data'".
