...
{
    /// <summary>
    /// Listens on TCP port 47998 for commands from the ArtLightControl Moonlight fork.
    /// 
    ...
    /// </summary>
    public sealed class ArtLightBridge : IDisposable
    {
        ...
        // ── Bridge authentication (ArtLightControl 7.1.0) ───────────────────────────
        ...
        public void Start()
        {
            if (_disposed) throw new ObjectDisposedException(nameof(ArtLightBridge));
            ...
            try
            {
                ...
                DebugLog($"ArtLightBridge listening on 0.0.0.0:{Port}");
            }
            catch (Exception ex)
            {
                DebugLog($"ArtLightBridge failed to start: {ex.Message}");
            }
        }
        ...
        private async Task ListenAsync(CancellationToken ct)
        {
            while (!ct.IsCancellationRequested)
            {
                try
                {
                    ...
                    {
                        DebugLog("ArtLightBridge: connection limit reached — rejecting peer");
                ...
                catch (Exception ex)
                {
                    if (!ct.IsCancellationRequested)
                        DebugLog($"ArtLightBridge accept error: {ex.Message}");
                }
            }
        }
        ...
        private async Task HandleClientAsync(TcpClient client, CancellationToken ct)
        {
            ...
            try
            {
                ...
                using (var writer = new StreamWriter(stream) { AutoFlush = true })
                {
                    ...
                    if (first.StartsWith("AUTH1 ", StringComparison.OrdinalIgnoreCase))
                    {
                        ...
                        if (!ok && RequireAuth)
                        {
                            DebugLog($"ArtLightBridge: rejected unauthorized command from {remote}");
                            await writer.WriteLineAsync("ERR_UNAUTHORIZED");
                            return;
                        }
                    ...
                    if (RequireAuth)
                    {
                        DebugLog($"ArtLightBridge: rejected unauthenticated '{first.ToUpperInvariant()}' from {remote} (RequireAuth on)");
                        await writer.WriteLineAsync("ERR_UNAUTHORIZED");
                        return;
                    }
            ...
            {
                DebugLog("ArtLightBridge: input exceeded size limit — connection dropped");
            }
            catch (Exception ex)
            {
                DebugLog($"ArtLightBridge client handler error: {ex.Message}");
            }
            finally
            {
            ...
            IPEndPoint? remote, IPEndPoint? local, string? clientId, bool authenticated, CancellationToken token)
        {
            ...
            switch (verb)
            {
                ...
                case "SETSPEED":
                {
                    ...
                    if (!authenticated)
                    {
                        DebugLog($"ArtLightBridge: rejected unauthenticated SETSPEED from {remote}");
                        await writer.WriteLineAsync("ERR_UNAUTHORIZED");
                        break;
                    }
                    if (LinkSpeed == null)
                    ...
                    {
                        DebugLog($"ArtLightBridge: malformed SETSPEED argument '{arg}' from {remote}");
                        await writer.WriteLineAsync("ERR");
                        break;
                    }
                ...
                case "UPDATECHECK":
                    ...
                    if (!authenticated)
                    {
                        DebugLog($"ArtLightBridge: rejected unauthenticated UPDATECHECK from {remote}");
                        await writer.WriteLineAsync("ERR");
                        break;
                    }
                ...
                case "UPDATE_NOW":
                    ...
                    if (!authenticated)
                    {
                        DebugLog($"ArtLightBridge: rejected unauthenticated UPDATE_NOW from {remote}");
                        await writer.WriteLineAsync("ERR");
                        break;
                    }
                ...
                case "SESSIONDATA":
                    ...
                    {
                        DebugLog("ArtLightBridge: failed to parse SESSIONDATA payload");
                        await writer.WriteLineAsync("ERR");
                    }
                    break;
