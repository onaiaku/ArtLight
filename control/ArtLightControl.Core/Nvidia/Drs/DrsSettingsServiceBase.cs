...
//   nvidiaProfileInspector/Common/DrsSettingsServiceBase.cs
// Changes for ArtLightControl:
//   • namespace → ArtLightControl.Nvidia.Drs; NVAPI types from ArtLightControl.Nvidia.Native.
//   • The DrsSettingsMetaService dependency was REMOVED. Upstream stores a `meta`
//     field here but never uses it in this base class (only derived display code
//     does), so dropping it lets us avoid porting the 500-line meta service for the
//     functional capture/restore core. Friendly value-name lookups are a later phase.
//   • Static ctor null-guards SYS_GetDriverAndBranchVersion so that merely touching
//     this type on a non-NVIDIA system cannot throw a TypeInitializationException.
using ArtLightControl.Nvidia.Native;
using System;
using System.Collections.Generic;
using System.Linq;
using System.Text;
using nvw = ArtLightControl.Nvidia.Native.NvapiDrsWrapper;
...
namespace ArtLightControl.Nvidia.Drs
{
    public abstract class DrsSettingsServiceBase
    {
        ...
        static DrsSettingsServiceBase()
        {
            ...
            // Guard: on AMD/Intel/no-driver systems the delegate is null.
            if (nvw.Instance.SYS_GetDriverAndBranchVersion != null &&
                nvw.Instance.SYS_GetDriverAndBranchVersion(ref sysDrvVersion, sysDrvBranch) == NvAPI_Status.NVAPI_OK)
            {
        ...
        public DrsSettingsServiceBase(DrsDecrypterService decrpterService = null)
        {
