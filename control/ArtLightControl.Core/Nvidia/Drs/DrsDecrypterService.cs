...
//   nvidiaProfileInspector/Common/DrsDecrypterService.cs
// Changes for ArtLightControl:
//   • namespace → ArtLightControl.Nvidia.Drs; NVAPI types from ArtLightControl.Nvidia.Native;
//     TempFile from ArtLightControl.Nvidia.Helper.
//   • ctor no longer takes DrsSettingsMetaService (base ctor takes none now).
//
// WHY THIS CLASS MATTERS: certain "internal" driver settings (some DLSS / Shader
// settings, e.g. "Pre-Compile Shader Options") are stored ENCRYPTED in the DRS
// database (XOR with the 256-byte key below). Reading them via the raw NVAPI value
// without decrypting yields garbage — that was the root cause of the old
// "(not set)" / "value never changes" bug when ArtLightControl used NvAPIWrapper.Net.
// CreateInternalSettingMap() discovers which settings are internal by dumping the
// DRS to a text file (DRS_SaveSettingsToFileEx) and matching `InternalSettingFlag=V0`.
...
namespace ArtLightControl.Nvidia.Drs
{
    public class DrsDecrypterService : DrsSettingsServiceBase
    {
        ...
        public DrsDecrypterService() : base()
        {
            try
            {
